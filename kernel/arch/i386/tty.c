...
void terminal_scroll(size_t lines) {
    if (lines >= VGA_HEIGHT) {
        ...
        /* Start at the line which will move off the screen */
        for (size_t y = lines; y < VGA_HEIGHT; y++) {
            for (size_t x = 0; x < VGA_WIDTH; x++) {
		const size_t old_index = y * VGA_WIDTH + x;
		const size_t new_index = (y - lines) * VGA_WIDTH + x;
                terminal_buffer[new_index] = terminal_buffer[old_index];
            }
        }
        /* Wipe all the lines that got shifted up */
...
void terminal_putchar(char c) {
    unsigned char uc = c;
    if (terminal_column == VGA_HEIGHT || uc == '\n') {
        terminal_column = 0;
        terminal_row++;
